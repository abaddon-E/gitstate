...
class Calverter:
    ...
    def jd_to_gregorian(self, jd):
        ...
        leapadj = 0 if wjd < self.gregorian_to_jd(
            year,
            3,
            1) else (1 if self.leap_gregorian(year) else 2)
    ...
    def iso_to_julian(self, year, week, day):
        ...
        return (
            day + self.n_weeks(0, self.gregorian_to_jd(year - 1, 12, 28), week)
        )
    ...
    def leap_jalali(self, year):
        ...
        return (
            ((((((year - 474 if year > 0 else 473) % 2820) + 474) + 38) * 682) %
             2816) < 682
        )
